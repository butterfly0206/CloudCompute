...
public class JSON {
    ...
    private static String getDiscriminatorValue(final JsonElement readElement, final String discriminatorField) {
        final JsonElement element = readElement.getAsJsonObject().get(discriminatorField);
        if (null == element) {
     ...
     */
    private static Class getClassByDiscriminator(final Map classByDiscriminatorValue, final String discriminatorValue) {
        final Class clazz = (Class) classByDiscriminatorValue.get(discriminatorValue);
        if (null == clazz) {
        ...
        public byte[] read(final JsonReader in) throws IOException {
            switch (in.peek()) {
                case NULL:
                    ...
                    return null;
                default:
                    final String bytesAsBase64 = in.nextString();
                    final ByteString byteString = ByteString.decodeBase64(bytesAsBase64);
